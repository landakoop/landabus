$(document).ready(() => {
	...
	const lineak = new Tabulator(".administrazioa-lineak__taula", {
	    ...
	    rowClick:function(e, row){
	    	...
	    	geltokiak.replaceData(":8080/api/geltokia/list4?lineaID="+row.getData().id);
	    	aukeratutakoak.replaceData(":8080/api/geltokia/list3?lineaID="+row.getData().id);
	    	ordutegiak.replaceData(":8080/api/linea/ordutegiak?lineaID="+row.getData().id);
	    }
	});
	
	const geltokiak = new Tabulator(".administrazioa-geltokiak__taula", {
	    ajaxURL: ":8080/api/geltokia/list4?lineaID=1",
	    ajaxConfig:"GET",
	    layout: "fitColumns",
	    tooltips: true,
	    addRowPos: "top",
	...
	const aukeratutakoak = new Tabulator(".administrazioa-aukeratutakoak__taula", {
	    ajaxURL: ":8080/api/geltokia/list3?lineaID=1",
	    ajaxConfig:"GET",
	    layout: "fitColumns",
	    tooltips: true,
	    addRowPos: "top",
	...
	const ordutegiak = new Tabulator(".administrazioa-ordutegiak__taula", {
	    ajaxURL: ":8080/api/linea/ordutegiak?lineaID=1",
	    ajaxConfig:"GET",
	    layout: "fitColumns",
	    tooltips: true,
	    addRowPos: "top",
	...
	$("#administrazioa-toolbar__new").click(function(){
		...
		$.ajax({
			...
			success: function(response) {
				...
				setTimeout(function(){
					lineak.replaceData(":8080/api/linea/");
					geltokiak.replaceData(":8080/api/geltokia/list4?lineaID=1");
			    	aukeratutakoak.replaceData(":8080/api/geltokia/list3?lineaID=1");
			    	ordutegiak.replaceData(":8080/api/linea/ordutegiak?lineaID=1");
				}, 500);
			}
		});		
	});
	...
	$("#administrazioa-toolbar__change").click(function(){
		...
		url = ":8080/api/linea/gehituGeltokiak" + lineakUrl + geltokiakUrl;
		console.log(url);
		$.post(url);
	...
	$("#administrazioa-ordutegia__form--new").click(function(){
		...
		setTimeout(function(){
	    	ordutegiak.replaceData(":8080/api/linea/ordutegiak?lineaID="+lineaId);
		}, 500);
	});
